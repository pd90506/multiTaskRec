...
def reindex(batch):
    '''Implement with sparse tensor to save memory when using large dataset'''
    batch_size = len(batch)
    new_batch = np.zeros(batch_size, dtype='long')
    new_index = 0
    #print(batch.unique())
    for i in batch.unique():
        index = np.where(batch == i)[0]
        new_batch[index] = new_index
        new_index += 1
    #print(new_batch)    
    return new_batch
...
class MovieLensDataset(Dataset):
    ...
    def __init__(self, csv_file):
        ...
        self.users = reindex(self.movielens.iloc[:, 0])
        self.items = reindex(self.movielens.iloc[:, 1])
        self.ratings = self.movielens.iloc[:, 2].astype('long')
