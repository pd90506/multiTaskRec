...
def reindex(batch):
    ...
    new_batch = np.zeros(batch_size, dtype=np.long)
    new_index = 0
    #print(batch.unique())
...
class MovieLensDataset(Dataset):
    ...
    def __init__(self, csv_file):
        ...
        self.users = self.movielens.iloc[:, 0]
        self.items = self.movielens.iloc[:, 1]
        self.ratings = self.movielens.iloc[:, 2].astype(int)
