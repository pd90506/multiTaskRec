...
from tensorflow.keras.layers import Embedding, Input, Dense, Reshape, Flatten, Dropout, Concatenate, Dot
...
def get_model(num_users, num_items, layers = [20,10], num_tasks=19, reg_layers=[0,0]):
    ...
    predictions = Dense(num_tasks, activation='sigmoid', kernel_initializer='lecun_uniform', name = 'predictions')(vector)
...
def fit(name_data = '100k', batch_size=2048):
    ...
    print("MMLP arguments: %s " %(args))
    model_out_file = 'Pretrain/%s_MMLP_%s_%d.h5' %(args.dataset, args.layers, time())
    result_out_file = 'outputs/%s_MMLP_%s_%d.csv' %(args.dataset, args.layers, time())
    ...
    # Build model
    model = get_model(num_users, num_items, layers, num_tasks, reg_layers)
    ...
    print("End MMLP. Best Iteration %d:  HR = %.4f, NDCG = %.4f. " %(best_iter, best_hr, best_ndcg))
